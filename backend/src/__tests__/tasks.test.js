...
describe('POST /api/tasks', () => {
  it('debe crear una tarea correctamente', async () => {
    const epiDataRes = await request(epiDataApp)
      .post('/api/tasks')
      .send({ title: 'Nueva tarea epi-data', description: 'Descripción', status: 'pendiente' });
  ...
  it('debe retornar 400 si falta el título', async () => {
    const epiDataRes = await request(epiDataApp).post('/api/tasks').send({ description: 'Sin título' });
  ...
  it('debe retornar 400 si el status es inválido', async () => {
    const epiDataRes = await request(epiDataApp)
      .post('/api/tasks')
      .send({ title: 'Tarea epi-data', status: 'invalido' });
    expect(epiDataRes.status).toBe(400);
  });
});
...
describe('PUT /api/tasks/:id', () => {
  it('debe actualizar una tarea existente', async () => {
    const epiDataTask = epiDataTaskService.create({ title: 'Original epi-data' });
    const epiDataRes = await request(epiDataApp)
      .put(`/api/tasks/${epiDataTask.id}`)
      .send({ title: 'Actualizada epi-data', status: 'completada' });
    expect(epiDataRes.status).toBe(200);
